...
Version 1.0.1 ships one canonical plugin with the MCP connection and shared
...
```sh
# Codex
codex plugin marketplace add robot-head/nodestorm --ref v1.0.1
codex plugin add nodestorm@nodestorm

# OpenCode
opencode plugin nodestorm@1.0.1 --global

# Pi
pi install npm:nodestorm@1.0.1
```
...
- Linux downloads the matching x64/arm64 v1.0.1 tarball, checks SHA-256 and
...
- macOS downloads the matching x64/arm64 v1.0.1 app, checks SHA-256,
...
node scripts/validate-release.mjs # synchronized v1.0.1 package contracts
